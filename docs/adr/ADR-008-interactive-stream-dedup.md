...
- `airing_key(name)` extracts the run's day, month, clock, meridiem and zone; `None` when the name states no **valid** airing. Only a real month name or abbreviation, a day in 1–31, a real clock time (12-hour when a meridiem is present, 24-hour otherwise) and complete tokens form an airing: `Sports 24 Hour 09:30` states none (`Hour` is not a month), `Channel 250 News 09:30` states none (`50` is inside `250`), `09:300` is not a clock. The name is scanned for the first *valid* run, so a false prefix (`SPORT 01 LIVE 09:30 …`) cannot hide a real trailing `@ 18 Sep 09:30 AM GMT-1`.
- A zone is one of the supported literal labels (`GMT`, `UTC`, `IST`, `BST`, `CET`, `EST`, …), bare or with a numeric offset (`GMT-1`, `UTC+05:30`), compared as written. `GMT` and `IST` are therefore different airings by policy (no conversion is attempted). Anything else after the clock — a quality tag such as `HD2`, a provider suffix — is not a zone and leaves the key unchanged, so it cannot veto a genuine duplicate.
- `find_candidate` drops a candidate before scoring **only** when both sides state a valid airing and the keys differ. A name with no valid airing is never dropped, so ordinary channel names — including ones that merely contain numbers and colons — score byte-for-byte as before, and the gate can only ever *remove* a candidate — it can never admit one the floor would refuse.
...
- The pattern has no unanchored whitespace scan: every gap is a bounded `\s{1,3}`, the day is anchored by a lookbehind boundary and the clock by a lookahead, and each start position does bounded work, so a whitespace-padded name costs linear time on top of the existing 512-character input cap.

**Companion change to the candidate scope (BD-F).** `_maybe_enqueue_pending_merge` builds its candidate list from the same-group, named channels first and only then excludes any surviving channel the incoming stream is **already attached to** (stream membership is never inspected on a channel that group or name eligibility has already ruled out). Such a pair scores 1.00 by construction (the channel's name is the stream's name), so leaving it in queues the stream as a merge against its own channel and defers a create that has nothing to review. Attachment is caller-side state, so the filter lives at the caller; the hook's contract that the caller owns candidate selection is unchanged.

**Observability (§D6/BD-M scope, no contract change).** `DedupHookResult` now carries the blocking `pending_merges` row id on **both** the fresh-insert and the §D5 collision branch. The pipeline result keeps three quantities apart: `pending_merges_added` counts deferred **create actions** (one stream with two create actions counts twice), `pending_merge_stream_count` counts the distinct **streams** whose channel was not created, and `pending_merge_ids` lists the distinct blocking **rows** (several streams can share one row). One shared phrase renders them in the engine completion log, the M3U-refresh task summary and — for a failed-action run — the single *Completed with Warnings* notification the task engine emits, whose metadata also carries the row ids. A deferral explanation is scoped to one create attempt: a later create attempt for the same stream supersedes it, so a follow-on `assign_epg` blames the most recent cause, not a stale deferral. A run that created nothing because every stream was deferred is no longer indistinguishable from a refresh where the provider had nothing.

**Dry-run visibility (issue #1015) is not in this change.** The preview path still predicts creation for a stream the live M3U-refresh path would defer against an existing pending row; whether that belongs in this fix or a follow-up is a maintainer decision recorded on the PR.
...
**Implementation status (bd-b3czq, v0.17.1-0075):** §D7 prompt-mode async-queue is now implemented as specified. The initial BD-P implementation deviated — `add_stream(dedup_action='prompt')` returned the candidate plus "re-call `add_stream` with `force_new`/`merge_if_found`" instructions instead of creating a `pending_merges` row, so `accept_channel_merge` / `dismiss_channel_merge` were unreachable from the add-stream flow. That deviation is resolved by **`POST /api/channel-merges`** (the enqueue endpoint), which async-queues the candidate (`trigger_context='mcp_tool'`, §D6 `auto_queued` journal row) and returns a `merge_id`. Both `prompt` and the `merge_if_found` below-threshold fallback now call it and hand the agent a real `merge_id` to accept/dismiss. The enqueue endpoint matches at the §D2 *floor* (so it surfaces every show-able candidate, mirroring the operator modal) and reports `meets_threshold` so a `merge_if_found` caller can decide auto-accept (at/above threshold → `accept_channel_merge`) vs prompt (below threshold → return the queued `merge_id`). The §D5 idempotency (a re-enqueued pair returns the existing `merge_id`), §D6 audit, and the BD-M queue-depth metric are shared with the bulk-M3U hook via a single `enqueue_pending_merge` core (`backend/services/m3u_dedup_hook.py`), so the two surfaces cannot drift.
